package com.kangaroo.sparring.domain.exercise.service;

import com.kangaroo.sparring.domain.exercise.entity.Exercise;
import com.kangaroo.sparring.domain.exercise.repository.ExerciseRepository;
...
public class ExerciseMatcher {
     ...
     * 4단계: 강도 기반 기본값 폴백
     */
    public MatchResult match(String exerciseName, ExerciseIntensityLevel intensity) {
    ...
    private Optional<Exercise> filterByIntensity(List<Exercise> candidates, ExerciseIntensityLevel intensity) {
        String intensityKeyword = switch (intensity) {
            case LOW    -> "가볍게";
            case MEDIUM -> "보통으로";
            case HIGH   -> "격렬하게";
        };
        return candidates.stream()
                .filter(e -> e.getExerciseName().contains(intensityKeyword)
                          || e.getIntensity() == intensity)
                .findFirst();
    }

    private double getFallbackMet(ExerciseIntensityLevel intensity) {
